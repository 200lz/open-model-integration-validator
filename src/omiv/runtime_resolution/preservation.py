"""Deterministic exhaustive Phase 6E baseline preservation discovery."""
...
from omiv.tokenizer_parity.preservation import EXHAUSTIVE_ROOTS as PHASE6D_ROOTS

BASELINE_REVISION = "77763002ef93ba3274145aa02ded476e488158ee"
EXHAUSTIVE_ROOTS = PHASE6D_ROOTS | {"tokenizer-configuration-parity"}
...
def audit_baseline(repository: Path) -> dict[str, Any]:
    ...
    for path in included:
        ...
        inventory.append(
            PreservationEntry(
                ...
                classification=(
                    "PHASE_6D"
                    if path.startswith("tokenizer-configuration-parity/")
                    else "PRIOR_PHASE"
                ),
    ...
    exclusions = tuple(
        ...
        for path in sorted(set(blobs) - set(included), key=str.encode)
    )
    inventory_json = [asdict(item) for item in inventory]
    return {
        "schema": "omiv.phase6e-preservation-audit-temporary.v1",
        ...
        "path_set_digest": canonical_sha256(
            {"domain": "omiv.phase6e-prior-artifact-path-set.v1", "paths": included}
        ),
        "inventory_digest": canonical_sha256(
            {"domain": "omiv.phase6e-prior-artifact-inventory.v1", "entries": inventory_json}
        ),
