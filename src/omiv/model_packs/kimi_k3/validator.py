...
def validate_layer_001(inventory: ModelInventory, schema: KimiK3Schema) -> ValidationFinding:
...
def validate_layer_002(inventory: ModelInventory, schema: KimiK3Schema) -> ValidationFinding:
    by_id = {layer.id: layer for layer in inventory.layers}
...
def validate_attn_001(inventory: ModelInventory, schema: KimiK3Schema) -> ValidationFinding:
    by_id = {layer.id: layer for layer in inventory.layers}
...
def validate_moe_001(inventory: ModelInventory, schema: KimiK3Schema) -> ValidationFinding:
    by_id = {layer.id: layer for layer in inventory.layers}
    ...
    passed = unanimous and contiguous and not incomplete and not coverage_count_mismatches
    evidence: dict[str, object] = {
...
def validate_moe_002(inventory: ModelInventory, schema: KimiK3Schema) -> ValidationFinding:
    by_id = {layer.id: layer for layer in inventory.layers}
    expected_moe = set(schema.expected_moe_layer_ids)
...
def validate_attn_002(inventory: ModelInventory, schema: KimiK3Schema) -> ValidationFinding:
    by_id = {layer.id: layer for layer in inventory.layers}
...
def validate_attnres_001(inventory: ModelInventory, schema: KimiK3Schema) -> ValidationFinding:
    by_id = {layer.id: layer for layer in inventory.layers}
    ...
    for component in schema.required_attention_residual_components:
        summary = inventory.semantic_descriptors.attention_residual_components.get(component)
    ...
    passed = not missing and not unexpected and not unexpected_layers and descriptors_ok
    return _finding(
        "K3-ATTNRES-001",
        passed,
...
def validate_attnres_002(inventory: ModelInventory, schema: KimiK3Schema) -> ValidationFinding:
    ...
    for component in schema.required_model_attention_residual_components:
        summary = inventory.semantic_descriptors.model_attention_residual_components.get(component)
        unanimous = _descriptor_is_unanimous(summary)
        descriptors_ok = descriptors_ok and unanimous
        descriptor_results[component] = {
            "unanimous": unanimous,
...
def validate_tensor_001(inventory: ModelInventory, schema: KimiK3Schema) -> ValidationFinding:
    ...
    groups.update(
        {
            f"shared_expert.{component}": semantic.shared_expert_components.get(component)
    ...
    groups.update(
        {
            f"routed_expert.{component}": semantic.routed_expert_components.get(component)
    ...
    summaries = {name: _descriptor_evidence(summary) for name, summary in sorted(groups.items())}
    return _finding(
        "K3-TENSOR-001",
        not inconsistent,
...
def validate_tensor_002(inventory: ModelInventory, schema: KimiK3Schema) -> ValidationFinding:
...
def validate_inventory(inventory: ModelInventory, schema: KimiK3Schema) -> ValidationReport:
