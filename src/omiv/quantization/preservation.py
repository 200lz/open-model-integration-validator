"""Deterministic exhaustive Phase 6C baseline preservation discovery."""
...
EXHAUSTIVE_ROOTS = PHASE6B_ROOTS | {"reconciliation"}
...
def audit_baseline(repository: Path) -> dict[str, Any]:
    ...
    path_digest = canonical_sha256(
        {"domain": "omiv.phase6c-prior-artifact-path-set.v1", "paths": included}
    )
    inventory_json = [asdict(x) for x in inventory]
    return {
        "schema": "omiv.phase6c-preservation-audit-temporary.v1",
        "baseline_revision": BASELINE_REVISION,
        "methodology": {
            "included_roots": sorted(EXHAUSTIVE_ROOTS),
            "rule": (
                "Every baseline blob under established generated roots plus all prior index "
                "members; no extension filter."
            ),
        },
        "counts": {
