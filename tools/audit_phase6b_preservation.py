"""Write deterministic temporary Phase 6B baseline preservation inventories."""
...
def main() -> None:
    ...
    audit = audit_baseline(args.repository)
    value = {
        **audit.json_value(),
        "reported_count_reconstruction": reconstruct_reported_counts(args.repository),
    }
    ...
    headings = (
        ...
        "current_size",
        "current_sha256",
        "phase_root_classification",
        "inclusion_reason",
    )
    rows = ["\t".join(headings)]
    rows.extend(
        "\t".join(str(getattr(item, heading)) for heading in headings) for item in audit.inventory
    )
    ...
    exclusion_rows.extend(
        "\t".join(str(getattr(item, heading)) for heading in exclusion_headings)
